...
var localHOC = function localHOC(config, WrappedComponent) {
	...
	return _temp2 = _class = function (_React$PureComponent) {
		...
		function _class() {
			...
			return _ret = (_temp = (_this = (0, _possibleConstructorReturn3.default)(this, (_ref2 = _class.__proto__ || Object.getPrototypeOf(_class)).call.apply(_ref2, [this].concat(args))), _this), _initialiseProps.call(_this), _temp), (0, _possibleConstructorReturn3.default)(_this, _ret);
		}
		...
		this.update = function (state, shouldReplace) {
			_this2.context.store.dispatch((0, _actions.update)(_this2.config.ns, state, shouldReplace));
		};

		this.reset = function () {
			var state = typeof _this2.config.state === 'function' ?
			// recalculate state on every reset if state is a function
			_this2.config.state(_this2.context.store.getState(), _this2.props) : _this2.config.state;

			_this2.update(state, true);
		};

		this.$ = function () {
			return (0, _actionEnhancer2.default)('ns', _this2.config.ns).apply(undefined, arguments);
		};
	}, _temp2;
};
